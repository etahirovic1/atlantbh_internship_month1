...
class US:
	...
	def verif_us(self):
		...
		yelp_hq = pd.read_json('/Users/admin/PycharmProjects/atlantbh_internship/high_quality.json', lines=True)
		canada_states = ['AB', 'BC', 'MB', 'NB', 'NL', 'NT', 'NS', 'NU', 'ON', 'PE', 'QC', 'SK', 'YT']
		yelp_hq_canada = yelp_hq[(yelp_hq['state'] == 'AB') | (yelp_hq['state'] == 'BC') | (yelp_hq['state'] == 'MB') | (yelp_hq['state'] == 'NB') | (yelp_hq['state'] == 'NL')
							| (yelp_hq['state'] == 'NT') | (yelp_hq['state'] == 'NS') | (yelp_hq['state'] == 'NU') | (yelp_hq['state'] == 'ON') | (yelp_hq['state'] == 'PE')
							| (yelp_hq['state'] == 'QC') | (yelp_hq['state'] == 'SK') | (yelp_hq['state'] == 'YT')]
		yelp_hq_canada = yelp_hq_canada.drop(['business_id', 'hours', 'stars', 'review_count', 'is_open', 'attributes', 'address', 'city', 'state', 'categories', 'postal_code'], axis=1)
		#yelp_hq_canada = yelp_hq_canada.head(200)	
		yelp_hq_canada['verification_score'] = yelp_hq_canada.apply(lambda x : self.skor(x), axis=1)
		yelp_hq_canada.to_csv("yelp_canada_hq_verified.csv", index=None, encoding='utf-8')


if __name__ == '__main__':
	file = pd.read_csv('canada-points-csv.csv', sep='\t')
